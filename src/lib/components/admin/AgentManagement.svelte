...
<div class="p-6">
	...
	<div class="flex items-center justify-between mb-6">
		<div>
			<h1 class="text-2xl font-bold text-gray-900 dark:text-white">
				{$i18n.t('Agent Management')}
			</h1>
			<p class="text-gray-600 dark:text-gray-400 mt-1">
				{$i18n.t('Manage all agents in the system')}
			</p>
		</div>
	...
	{:else}
					...
					<tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
						{#each filteredAgents as agent (agent.id)}
							<tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
								...
								<td class="px-6 py-4 whitespace-nowrap">
									...
									{#if agent.config?.klAssistId}
										<div class="text-xs text-gray-500 dark:text-gray-400">
											{$i18n.t('Knowledge Assistant')}: {agent.config.klAssistId}
										</div>
									{/if}
								</td>
								<td class="px-6 py-4 whitespace-nowrap">
								...
								<td class="px-6 py-4 whitespace-nowrap">
									<span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {agent.enabled 
		...
		<div class="mt-4 text-sm text-gray-500 dark:text-gray-400">
			{$i18n.t('Showing')} {filteredAgents.length} {$i18n.t('of')} {agents.length} {$i18n.t('agents')}
		</div>
	{/if}
</div>
